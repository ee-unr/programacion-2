...
2.  **Mover:** Deslizar una caja una posición a la izquierda o derecha. **(1 paso por caja movida)**
...
Para cada escenario, calculá el costo en pasos y deducí su complejidad asintótica (Big O), basándote en la analogía física.
...
Un DJ está gestionando su lista principal de canciones, almacenada en una secuencia ordenada de reproducción. Necesitas desarrollar las herramientas para manipular esta lista de forma eficiente y precisa antes del show.
...
- Mostra al DJ la lista completa, indicando claramente la posición inicial de cada pista en la secuencia.
...
`leer_pos(i)`: Devuelve el turno en la posición `i`.
...
Devuelve la posición (índice) donde está ese horario, o `-1` si no existe.

`listar()`: Devuelve una lista de strings con el formato:
`["0) 08:30 - Juana Pérez", "1) 09:00 - Mario Gómez", ...]`
...
¿Qué ocurre con los índices cuando eliminás o insertás un turno en el medio de la lista?
(Pista: ¿qué pasa con los elementos que estaban después?)

¿Por qué acceder a agenda[i] es una operación O(1)
pero buscar un turno por hora es O(N)?

¿Qué estructura te parecería más conveniente si hubiera miles de turnos?
Explicá brevemente por qué.

## Torre de control Aereo ✈️ 
