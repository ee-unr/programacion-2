...
```python
def _quick_sort(arr, inicio=0, fin=None):
...
def particionar(arr, inicio, fin):
    pivote = arr[fin]          # Tomamos el último elemento como pivote
    i = inicio - 1             # i marca el límite de los menores al pivote

    for j in range(inicio, fin):
        if arr[j] <= pivote:   # Si el valor actual es menor o igual al pivote
            i += 1
...
def quick_sort(arr):
    arr_copia = arr[:]
    _quick_sort(arr_copia) # Ordena _in place_
    return arr_copia
```
