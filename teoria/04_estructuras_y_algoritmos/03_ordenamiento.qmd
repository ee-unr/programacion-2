...
Almacenar un conjunto de datos de manera ordenada permite realizar búsquedas y otras
operaciones de forma más eficiente.
...
que realizan diferentes operaciones y se basan en distintas estrategias con un mismo 
objetivo: obtener una secuencia de valores ordenados.
...
Más formalmente, si tenemos una secuencia $\boldsymbol{S} = [s_1, s_2, \dots, s_n]$, 
diremos que está ordenada si se cumple que $s_i \le s_{i + 1}$ para todo 
$i \in \{1, 2, \dots, n - 1\}$.
...
en el segundo, el valor `60` tiene a su derecha un número menor (`10`), lo que rompe el 
orden.

Una forma sencilla de obtener un algoritmo de ordenamiento consiste en comparar cada valor con el siguiente. Si el siguiente es menor, se intercambian; si no, se mantienen en su lugar.
...
Ahora, debemos notar que obtuvimos una lista ordenada en tan pocos pasos de pura suerte.
...
En la segunda, la lista quedó más ordenada que al inicio, pero aún no completamente.

Como mencionamos anteriormente, que en el primer caso se obtenga una lista ordenada al
realizar una sola pasada fue simplemente una coincidencia.

El algoritmo _bubble sort_ funciona realizando pasadas sucesivas sobre la lista,
...
def bubble_sort(arr):
    arr = arr[:] # Hacer copia
    n = len(arr)
    intercambiado = True  # <1>

    while intercambiado:  # <2>
        intercambiado = False  # <3>
        for i in range(n - 1): # <4>
            if arr[i] > arr[i + 1]: # <5>
                # Se intercambian los valores # <5>
                arr[i], arr[i + 1] = arr[i + 1], arr[i] # <5>
                intercambiado = True # <5>

    return arr
```

1. Se usa una variable booleana `intercambiado` para indicar si en la pasada anterior se hizo
un intercambio de elementos, y por lo tanto se debe realizar una pasada más. Inicialmente,
este valor es `True` para que se ejecute el bucle `while` al menos una vez.
2. Mientras se haya intercambiado en la pasada anterior.
3. El primer paso de cada pasada es igualar `intercambiado` a `False`. 
Solo cuando se haga un intercambio se lo convierte a `True`.
4. Se itera a través de todos los valores en la secuencia, excepto el último, ya que se 
comapran de a pares.
5. Si el valor de la posición `i` es mayor que el de la posición `i + 1`, se intercambian
y se iguala `intercambiado` a `True`.
...
En cada pasada, el algoritmo ubica el mayor valor fuera de orden en su posición correcta,
al final de la lista.
Este proceso hace que los valores más grandes "suban" gradualmente hacia la parte superior,
como burbujas que ascienden en el agua, lo que da origen al nombre _bubble sort_.
...
Además, en el peor de los casos (cuando la lista está ordenada en forma descendente),
...
Así, se concluye que la complejidad temporal de _bubble sort_ $O(N^2)$.
Aunque la cantidad de pasos no solo dependa $N^2$, sino también de $N$, 
la notación _Big O_ se centra en el orden del término dominante y, en este caso, se dice
que _buble sort_ es de complejidad cuadrática.

La siguiente tabla muestra lo rápido que crece la cantidad máxima de pasos conforme se
incrementa la cantidad de elementos a ordenar:
...
def selection_sort(arr):
    ...
    for i in range(len(arr) - 1): # <1>
        indice_menor_numero = i   # <2>

        for j in range(i + 1, len(arr)):           # <3>
            if arr[j] < arr[indice_menor_numero]:  # <3>
                indice_menor_numero = j            # <3>

        if indice_menor_numero != i: # <4>
            arr[i], arr[indice_menor_numero] = arr[indice_menor_numero], arr[i] # <4>

    return arr
```

1. El puntero `i` itera desde el primer hasta el penúltimo elemento. Este es el que
realiza las pasadas.
2. En cada pasada, se toma al primer valor como el mínimo, por eso `indice_menor_numero` se iguala a `i`.
3. Otro puntero, `j`, recorre la lista a partir de `i`, hasta el final. 
Si el valor en la posición `j` es mejor que en el mínimo actual, 
se toma a `j` como el índice con el mínimo.
4. Si el mínimo no está al principio de la sublista que se recorre, se intercambian
las posiciones de los elementos para que el mínimo esté al principio.
...
Al igual que _bubble sort_, el algoritmo _selection sort_ también utiliza dos tipos de 
operaciones: comparaciones e intercambios. En cada pasada, se compara cada valor de la 
parte desordenada con el mínimo actual, y al finalizar se coloca dicho mínimo en su 
posición correcta (intercambiándolo con el valor que estaba allí).

La cantidad de comparaciones que realiza es idéntica a la del algoritmo _bubble sort_.
Para un arreglo de longitud $N$, se efectúan $\frac{N^2}{2} - \frac{N}{2}$ comparaciones.

Por otro lado, y a diferencia del algoritmo _bubble sort_, se realiza **a lo sumo** un 
intercambio por pasada, por lo que en total pueden realizar hasta $N - 1$ intercambios y 
la cantidad máxima de pasos del algoritmo _insertion sort_ es:

$$
\frac{N^2}{2} - \frac{N}{2} + (N - 1),
$$

lo que indica que la complejidad temporal de _selection sort_ también es cuadrática.
...
A pesar de que el algoritmo _selection sort_ realiza aproximadamente la mitad de pasos que
_bubble sort_, ambos se consideran algoritmos de complejidad cuadrática.
...
def insertion_sort(arr):
    ...
    for i in range(1, len(arr)): # <1>
        ...
        puntero = i - 1 # <2>
        while puntero >= 0 and arr[puntero] > valor_i: # <3>
            arr[puntero + 1] = arr[puntero] # <3>
            puntero -= 1 # <3>

        arr[puntero + 1] = valor_i # <4>

    return arr
```

1. La variable `i` es la posición original del elemento a insertar en la lista ordenada. 
Se empieza del segundo elemento. Su valor lo representa `valor_i`.
2. Un `puntero` recorre de derecha a izquierda los elementos a la izquierda de la posición `i`.
3. Si el puntero no llegó al final y el valor en su posición es mayor al valor a insertar
en la sublista ordenada (`valor_i`), se mueve el valor en la posición del puntero a la 
derecha y se decrementa el puntero en 1 unidad.
4. Una vez que termina el proceso de traslación de valores, se inserta el `valor_i` en 
la posición que le corresponde, que es a la derecha del `puntero`.
...
El algoritmo _insertion sort_ realiza cuatro tipos de operaciones: 
extracciones, comparaciones, desplazamientos e inserciones.

Las comparaciones ocurren cada vez que se compara el `valor_actual` con los elementos que
tiene a su izquierda. En el peor de los casos, cuando la lista está ordenada en sentido 
descendente, todos los valores a la izquierda son mayores que `valor_actual`, 
lo que obliga con compararlo con todos ellos antes de volver a insertarlo.
...
Hasta ahora, el análisis se centró siempre en el peor escenario,
pero el desempeño de _insertion sort_ depende en gran medida del orden inicial de los datos.

En el peor de los casos, cuando el arreglo está ordenado de manera decreciente, 
_insertion sort_ tiene una complejidad de $O(N^2)$.
En esta situación, su desempeño resulta inferior al de _selection sort_, 
que requiere menos operaciones (alrededor de $O(N^2/2)$).

Si los datos ya están ordenados, _insertion sort_ solo realiza una comparación por pasada
y ningún desplazamiento, sumando en total $N - 1$ comparaciones.
_Selection sort_, por su parte, efectuaría igualmente unas 
$\frac{N^2}{2} - \frac{N}{2}$ comparaciones.
...
2. Ordenar (Conquistar): aplicar recursivamente _merge sort_ sobre $S_1$ y $S_2$.
...
3. Se ejecuta un bucle hasta que el puntero izquierdo o el puntero derecho lleguen al final de su respectivas secuencias. Dentro del bucle, realiza lo siguiente:
   ...
   índice de su arreglo. Si en algún momento los dos valores que se comparan son iguales, se puede añadir arbitrariamente el valor del arreglo `izq` e incrementar su puntero.
...
En la práctica, suele seleccionarse el último elemento o uno al azar (que se mueve al final).
...
Hacer [clic aquí](https://opendsa-server.cs.vt.edu/embed/quicksortAV).
