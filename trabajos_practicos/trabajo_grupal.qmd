...
1. `archivo` (`str` o `None`): nombre del archivo donde se escribirán los mensajes de monitoreo.
...
* `{funcion}`: nombre de la función y el nombre del archivo donde está definida, separados por `::`.
* `{modulo}`: nombre del módulo desde el cual se ejecuta la función.
...
```python
formato = "({tiempo}) {funcion}[{argumentos}] demoró {duracion} milisegundos."
# (2025-10-08 11:54:12.123) utilidades::fun[a=1, b=2, c=3] demoró 735 milisegundos.

formato = "({tiempo}) ({modulo}) {funcion}[{argumentos}] demoró {duracion} milisegundos."
# (2025-10-08 11:54:12.123) (__main__) utilidades::fun[a=1, b=2, c=3] demoró 735 milisegundos.
```
