...
1. Tenga en cuenta el módulo estándar `inspect` para interactuar con los parámetros de la función
decorada. En particular, considere:
    ...
    # Obtener 'firma' de la funcion
    firma = inspect.signature(fun)

    # Pasarle ("bindearle") valores '1' y '2' a la llamada a la funcion
    argumentos = firma.bind(1, 2)
    print(argumentos)

    # Agregar los valores por defecto a la firma
    argumentos.apply_defaults()
    print(argumentos)

    # Obtener los parametros y sus valores
    print(argumentos.arguments.items())
    ```

2. Dado que el decorador tiene que poder usarse de las siguientes dos maneras:
...
3. Considere el método `.format` de las cadenas de caracteres para usar el `formato` pasado al
decorador.
4. Enlaces útiles:
    ...
    * [Fábrica de decoradores](../practica/02_programacion_funcional/ejercicios.qmd#bromas-pesadas)