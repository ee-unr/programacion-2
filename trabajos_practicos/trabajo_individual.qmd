...
| `get_column(self, name)`       | Devuelve la `Series` asociada a la columna `name`.                               |
...
| `sort(self, name, descending)` | Ordena las filas según la columna `name`. Por defecto, `descending` es `False`. |
...
| `__getitem__(self, index)` | Devuelve un diccionario con los valores correspondientes a la fila `index`. |
...
Por su parte, `__getitem__` permite acceder a una fila por su índice, devolviendo un diccionario donde las claves son los nombres de las columnas y los valores, los elementos correspondientes a esa fila:
...
Cuando el `DataFrame` contiene más de diez filas, se muestran las primeras cinco y las últimas cinco, separadas por puntos suspensivos (`...`).
...
El nombre del archivo comprimido debe seguir el formato: `{Apellido}_{Nombre}.zip`. Por ejemplo: `Capretto_Tomas.zip`.
